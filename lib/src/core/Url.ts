...
export default class Url {
    ...
    public fromString(url: string) {
        ...
        if (url.indexOf(".") >= 0 || url.indexOf("localhost") >= 0) {
            ...
            const splitDomain = url.split("/");
            if (splitDomain.length > 1) {
                this._path = splitDomain.slice(1).join("/");
            }
            this._domain = splitDomain[0];
        } else {
            // path only
            if (url.startsWith("/")) {
                url = url.substr(1);
    ...
    public get https(): boolean { return this._https; }
    public set https(value: boolean) { this._https = value; }
    ...
    public get subdomain(): string {
        const splitDomain = this._domain.split(".");
        if (splitDomain.length > 1) {
    ...
    public get query(): object { return this._query; }
    public set query(value: object) { this._query = value; }
